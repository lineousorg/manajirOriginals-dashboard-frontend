...
// Type for form that allows optional attributes
interface ProductFormDataWithOptionalAttrs {
  name: string;
  description: string;
  categoryId: number;
  variants: {
...
const EditProductPage = () => {
  ...
  useEffect(() => {
    if (product) {
      // Transform backend attribute format to frontend format
      const transformAttributes = (variant: (typeof product.variants)[0]) => {
        if (!variant.attributes || variant.attributes.length === 0) {
          console.log("No attributes for variant:", variant.sku);
          return [];
        }
        const transformed = variant.attributes.map((attr: any) => {
          const result = {
            attributeId:
              attr.attributeValue?.attribute?.id ?? attr.attributeId ?? 0,
            valueId: attr.attributeValue?.id ?? attr.valueId ?? 0,
          };
          console.log("Transformed attr:", variant.sku, attr, "->", result);
          return result;
        });
        console.log("Transformed attributes for", variant.sku, ":", transformed);
        return transformed;
      };
      ...
      reset({
        ...
        variants: product.variants.map((v) => ({
          ...
          stock: v.stock,
          attributes: transformAttributes(v),
        })),
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    const formDataTyped = data as ProductFormDataWithOptionalAttrs;

    try {
      const updateData = {
        name: formDataTyped.name,
        description: formDataTyped.description,
        categoryId: formDataTyped.categoryId,
        variants: formDataTyped.variants.map((v) => ({
          sku: v.sku,
          price: v.price,
          stock: v.stock,
          attributes: (v.attributes || [])
            .filter(
              (a) => a.attributeId !== undefined && a.valueId !== undefined
            )
            .map((a) => ({
  ...
  const handleToggleVariantActive = async (variantId: number) => {
    ...
    try {
      ...
      toast({
        title: currentVariant?.isActive
          ? "Variant deactivated"
          : "Variant activated",
        description: `Variant has been ${
          currentVariant?.isActive ? "deactivated" : "activated"
        } successfully.`,
      });
    } catch (error) {
      toast({
                      ...
                      <div className="flex items-center gap-2 h-10">
                        {product.variants[index]?.id ? (
                          <>
                            <Switch
                              checked={
                                product.variants[index]?.isActive ?? true
                              }
                              onCheckedChange={() =>
